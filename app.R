...
ui <- dashboardPage( title = "Chromatin changes in mitosis",
      ...
      tabItems(
        ...
        tabItem(tabName = "LandingPage",
                ...
                fluidRow( column(1),
                          column(9,
                                 tags$h4(tags$i("Itaru Samejima, Christos Spanos, Kumiko Samejima, Juri Rappsilber, Georg Kustatscher and William C. Earnshaw")),
                                 tags$h5(tags$i(tags$a(href = "https://pubmed.ncbi.nlm.nih.gov/35090599/", "Molecular Cell, 2022, Feb 3;82(3):696-708 (Open Access)"))),
                                 br(),
        ...
        tabItem(tabName = "T1",
                fluidRow( 
                  box( solidHeader = TRUE, background = "navy", width = 3, 
                       br(),
                       pickerInput(inputId = "highlight_1",
                                   label = "Select protein(s) to highlight",
                                   choices =  DT9$plot_label,
                                   options = pickerOptions( actionsBox = TRUE,
                                                            title = "Search by gene name, protein name or UniProt ID", 
                                                            liveSearch = TRUE,
                                                            liveSearchPlaceholder = "e.g. SMC2",
                                                            dropupAuto = FALSE,
                                                            selectedTextFormat = "count",
                                                            multipleSeparator = " | ",
                                                            noneResultsText = "No proteins found. Could it have a different UniProt ID?",
                                                            showSubtext = FALSE),
                                 multiple = TRUE),
                       ...
                       fluidRow(
                         column(12,
                                box( solidHeader = TRUE, background = "navy", width = NULL,
                                     ...
                                     pickerInput(inputId = "cluster_to_highlight", 
                                                 label = tags$p( "Option A: by ID"),
                                                 choices = unique( DT9[, hier_cl_averag_1 ] ),
                                                 options = pickerOptions( actionsBox = TRUE,
                                                                          dropupAuto = FALSE,
                                                                          selectedTextFormat = 'static',
                                                                          title = "Select clusters by their ID"),
                                                 multiple = TRUE),
                                     pickerInput(inputId = "protein_to_highlight_in_cluster", 
                                                 label = "Option B: by protein",
                                                 choices = DT9[ !is.na( hier_cl_averag_1 ), plot_label ], 
                                                 options = pickerOptions( liveSearch = TRUE,
                                                                          liveSearchPlaceholder = "Search by gene name, protein name or UniProt ID",
                                                                          dropupAuto = FALSE,
                                                                          actionsBox = TRUE,
                                                                          noneResultsText = "No proteins found. Could it have a different UniProt ID?",
                                                                          showSubtext = FALSE,
                                                                          selectedTextFormat = 'static',
                                                                          title = "Select proteins to show the associated clusters"),
                                                 multiple = TRUE)
                                     )))),
...
server <- function(input, output, session) {
  ...
  observeEvent( input$delete_trace , {                  # When delete_trace button is triggered
    ...
    updatePickerInput( session, inputId = "highlight_1", selected = character(0))  # And also update the selected proteins in the drop down menu
    })
  ...
  observe({
    ...
    updatePickerInput( session, inputId = "highlight_1", selected = t_SNE_click_data$key )  # And also update the selected proteins in the drop down menu
    })
  ...
  observe({
    ...
    updatePickerInput( session, inputId = "highlight_1", selected = t_SNE_selected_data$key )
  })
  ...
  ## Reactive updating of clusters shown ##
    ...
    observeEvent(input$cluster_to_highlight, { 
      ...
      updatePickerInput( session, inputId = "protein_to_highlight_in_cluster",  selected = character(0) )  # And re-set Option B, so that previously selected proteins become deselected
    ...
    observeEvent(input$protein_to_highlight_in_cluster, {
      ...
      updatePickerInput( session, inputId = "cluster_to_highlight", selected = character(0)  )   # At the same time, we need to re-set the menu for Option A
  ...
  ## Dropdown menu behaviour ## 
    ...
    observeEvent(rv$current_cl_height, {
      ...
      updatePickerInput( session, inputId = "cluster_to_highlight", choices = choices )    # If cluster height changes, update dropdown menu choices for cluster IDs
      updatePickerInput( session, inputId = "protein_to_highlight_in_cluster",             # If cluster height changes, update dropdown menu choices for selection via protein IDs
